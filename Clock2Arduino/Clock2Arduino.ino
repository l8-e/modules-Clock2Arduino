...
void loop() {
  ...
  bool externalClockIsHigh = (digitalRead(CLOCK_IN) == LOW); // note that the guard inverts the signal. The transistor pulls down the input level when the clock signal is high.
  handleInternalClock(t);

  if(externalClockIsHigh && !externalClockWasHigh){
    if(!SYNC_LED_TO_INTERNAL){
      digitalWrite(LED_OUT,HIGH);
    }
